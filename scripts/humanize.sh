...
# Monitor the latest Codex run log from .humanize-rlcr.local
...
_humanize_monitor_codex() {
    local loop_dir=".humanize-rlcr.local"
    local current_file=""
    ...
    # Check if .humanize-rlcr.local exists
    if [[ ! -d "$loop_dir" ]]; then
    ...
    _find_latest_codex_log() {
        ...
        # First, find valid session timestamps from local .humanize-rlcr.local
        for session_dir in "$loop_dir"/*; do
            # Skip if glob didn't match anything
            [[ ! -e "$session_dir" ]] && continue
            # Only process directories
