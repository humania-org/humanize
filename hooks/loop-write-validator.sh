...
if [[ "$IN_PR_LOOP_DIR" == "true" ]]; then
    ...
    # - round-N-pr-comment.md (fetched comments, generated by system)
    # - round-N-prompt.md (prompts from system)
    # - round-N-codex-prompt.md (Codex prompts, generated by stop hook)
    # - round-N-pr-check.md (Codex output, generated by stop hook)
    # - round-N-pr-feedback.md (feedback for next round, generated by stop hook)
    if is_pr_round_file_type "$FILE_PATH_LOWER" "pr-comment" || \
       is_pr_round_file_type "$FILE_PATH_LOWER" "prompt" || \
       is_pr_round_file_type "$FILE_PATH_LOWER" "codex-prompt" || \
       is_pr_round_file_type "$FILE_PATH_LOWER" "pr-check" || \
       is_pr_round_file_type "$FILE_PATH_LOWER" "pr-feedback"; then
        ...
        if [[ -n "$ACTIVE_PR_LOOP_DIR" ]]; then
            ...
            if [[ -f "$PR_STATE_FILE" ]]; then
                ...
                if [[ -n "$CLAUDE_PR_ROUND" ]] && [[ "$CLAUDE_PR_ROUND" != "$PR_CURRENT_ROUND" ]]; then
                    ...
                    echo "" >&2
                    echo "You tried to write to round-${CLAUDE_PR_ROUND}-pr-resolve.md but current PR loop round is **${PR_CURRENT_ROUND}**." >&2
                    echo "" >&2
                    echo "Write to: \`$CORRECT_PATH\`" >&2
                    exit 2
                fi
            fi
        fi
